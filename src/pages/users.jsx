    ...
    <div className={`flex-1 flex flex-col h-full overflow-hidden ${isDark ? "bg-gray-950" : "bg-[#f8fafc]"}`}>
      ...
      <div className={`px-6 py-5 border-b flex flex-col md:flex-row md:items-center justify-between gap-4 ${isDark ? "bg-gray-950 border-gray-900" : "bg-white border-gray-100"
        ...
        <div>
          ...
          <p className={`text-sm mt-0.5 ${isDark ? "text-gray-500" : "text-gray-500"}`}>
            Manage access and roles for {users.length} platform users
          </p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          {[
            { label: 'Total Accounts', value: users.length, icon: IconUsers, color: 'blue' },
            { label: 'Admins', value: users.filter(u => u.role === 'admin').length, icon: IconCheck, color: 'emerald' },
            { label: 'Regular Users', value: users.filter(u => u.role !== 'admin').length, icon: IconX, color: 'rose' },
          ].map((stat, i) => (
            ...
            <table className="w-full text-left whitespace-nowrap">
              ...
              <tbody className={`divide-y ${isDark ? "divide-gray-800" : "divide-gray-100"}`}>
                {loading ? (
                  ...
                  <tr>
                    <td colSpan="6" className="py-20 text-center text-gray-500 font-bold uppercase tracking-widest">No Users Found</td>
                  </tr>
                ) : (
                  filteredUsers.map((user, i) => (
                    <tr key={user._id || i} className={`group transition-all duration-300 ${isDark ? "hover:bg-gray-800/40" : "hover:bg-gray-50/70"}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className={`w-10 h-10 rounded-xl bg-blue-500 flex items-center justify-center text-white text-xs font-black shadow-lg shadow-black/10`}>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          <span className={`text-xs font-bold ${isDark ? "text-gray-400" : "text-gray-700"}`}>{user.role || 'user'}</span>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className={`text-xs font-medium ${isDark ? "text-gray-400" : "text-gray-600"}`}>{user.address || 'Not set'}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-tighter shadow-sm ${user.role === "admin"
                          ? (isDark ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-600')
                          : (isDark ? 'bg-blue-500/10 text-blue-400' : 'bg-blue-50 text-blue-600')
                          }`}>
                          <div className={`w-1 h-1 rounded-full ${user.role === 'admin' ? 'bg-emerald-500' : 'bg-blue-500'}`}></div>
                          {user.role === 'admin' ? 'Admin' : 'User'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`text-xs font-medium ${isDark ? "text-gray-500" : "text-gray-400"}`}>{new Date(user.createdAt).toLocaleDateString()}</span>
                      </td>
